package com.cloudok.authority.swagger;
...
public class AuthoritySwaggerConfigure {
	...
	@Bean
	public Docket authoritySwagger() {
		...
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo).groupName("authority").select()
				.apis(!("dev".equals(active) || "local".equals(active) || "test".equals(active))
						? RequestHandlerSelectors.none()
						: RequestHandlerSelectors.basePackage("com.cloudok.authority"))
