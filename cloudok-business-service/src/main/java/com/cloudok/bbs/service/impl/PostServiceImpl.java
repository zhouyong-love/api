...
//	private SpecialismService specialismService;
	...
	public TopticPostResult getPostsByTopic(Long topicId, Integer topicType, Integer pageNo, Integer pageSize) {
		...
		Page<PostVO> page = this.page(QueryBuilder.create(PostMapping.class)
				.and(PostMapping.topicId, topicId).and(PostMapping.topicType, topicType).end()
				.sort(PostMapping.CREATETIME).desc()
	...
	private void fillPostInfo(List<PostVO> postList) {
		...
		this.fillPostThumbsUp(postList.stream().filter(item -> item.getThumbsUpCount()!=null && item.getThumbsUpCount() != 0 && item.getLatestComment() == null).collect(Collectors.toList()), 5);
	...
	private void fillPostThumbsUp(List<PostVO> postList,int size) {
		...
		List<ThumbsUpVO> thumbsUpList = this.thumbsUpService.list(QueryBuilder.create(ThumbsUpMapping.class).and(ThumbsUpMapping.BUSINESSID, QueryOperator.IN,postIdList)
				.end().sort(ThumbsUpMapping.CREATETIME).desc()
				.enablePaging().page(1, postList.size()*100).end()
			);
	...
	public PostVO getDetails(Long id) {
		...
		//查询最新的10个点赞
		List<ThumbsUpVO> thumbsUpList = this.thumbsUpService.list(QueryBuilder.create(ThumbsUpMapping.class).and(ThumbsUpMapping.BUSINESSID, id)
				.end().sort(ThumbsUpMapping.CREATETIME).desc()
				.enablePaging().page(1, 10).end()
				);
		List<Long> memberIdList = new ArrayList<Long>();
		if(!CollectionUtils.isEmpty(thumbsUpList)) {
...
//		List<TagVO> tagList = this.tagService.list(QueryBuilder.create(TagMapping.class).sort(TagMapping.SN).desc());
//		tagList.stream().forEach(item ->{
//			TopicVO topic = null;
//			if(TagCategory.systemTopic.getValue().equals(item.getCategory())) {
//				topic = new TopicVO(item.getId(),BBSTopicType.system,item.getName(),item.getIcon(),item.getSn());
//			}
//			if(TagCategory.personality.getValue().equals(item.getCategory())) {
//				topic = new TopicVO(item.getId(),BBSTopicType.personalityTag,item.getName(),item.getIcon(),item.getSn());
//			}
//			if(TagCategory.statement.getValue().equals(item.getCategory())) {
//				topic = new TopicVO(item.getId(),BBSTopicType.statementTag,item.getName(),item.getIcon(),item.getSn());
//			}
//			if(topic != null) {
//				topicList.add(topic);
//			}
//		});
//		List<SchoolVO> schoolList = this.schoolService.list(QueryBuilder.create(SchoolMapping.class).sort(SchoolMapping.SN).desc());
//		schoolList.stream().forEach(item ->{
//			TopicVO   topic = new TopicVO(item.getId(),BBSTopicType.school,item.getName(),item.getSn());
//			topicList.add(topic);
//		});
//		
//		List<SpecialismVO> specialismList = this.specialismService.list(QueryBuilder.create(SpecialismMapping.class).sort(SpecialismMapping.SN).desc());
//		specialismList.stream().forEach(item ->{
//			TopicVO   topic = new TopicVO(item.getId(),BBSTopicType.specialism,item.getName(),item.getSn());
//			topicList.add(topic);
//		});
//		
//		List<ResearchDomainVO> researchList = this.researchDomainService.list(QueryBuilder.create(ResearchDomainMapping.class).sort(ResearchDomainMapping.SN).desc());
//		researchList.stream().forEach(item ->{
//			TopicVO   topic = new TopicVO(item.getId(),BBSTopicType.researchDomain,item.getName(),item.getSn());
//			topicList.add(topic);
//		});
//		List<DictDataVO> industryList = dictService.findAllFromCache("experience");
//		industryList.stream().forEach(item ->{
//			TopicVO   topic = new TopicVO(item.getId(),BBSTopicType.researchDomain,item.getDictShowName(),item.getSn().intValue());
//			topicList.add(topic);
//		});
